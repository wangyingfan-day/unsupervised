...
def experiment(exp, arch, rnd_init, img_size, confidence_thresh, teacher_alpha, unsup_weight,
    ...
    import image_dataset, visda17_dataset
    ...
    with torch.cuda.device(device):
        ...
        net1 = net_class(n_classes, img_size, use_dropout, not rnd_init).cuda()

        net = byol.BYOL(net1,img_size,hidden_layer='avgpool')
        ...
        if rnd_init:
            ...
            for name, param in net_params:
                ...
                elif name.startswith('net.new_'):
                    new_params.append(param)
                else:
                    fix = False
                    for lyr in fix_layers:
                        if name.startswith('net'+lyr + '.'):
                            fix = True
                            break
                    if not fix:
        ...
        def f_train(X_sup, y_sup, X_unsup0, X_unsup1):
            ...
            net.train(mode=True)
            ss_loss,online_log_one,target_log_two,sup_logits = net(X_sup,X_unsup0,X_unsup1)


    
            student_unsup_prob_out = F.softmax(online_log_one,dim=1)
   
            teacher_unsup_prob_out = F.softmax(target_log_two,dim=1)

            # Supervised classification loss
  
            if double_softmax:
                clf_loss = classification_criterion(F.softmax(sup_logits,dim=1), y_sup)
            else:
                clf_loss = classification_criterion(sup_logits, y_sup)
            ...
            loss_expr = loss_expr + ss_loss * unsup_weight 
            ...
            mask_count = conf_mask_count.data.cpu()
            

            outputs = [float(clf_loss.data.cpu()) * n_samples,
                       float(unsup_loss.data.cpu()) * n_samples,
                       mask_count]
        ...
        def f_pred_src(X_sup):
            ...
            _,_,out = net.target_encoder(X_var)
            return (F.softmax(out).data.cpu().numpy(),)
        ...
        def f_pred_tgt_mult(X_sup):
            ...
            for aug_i in range(len(X_sup)):
                ...
                _,_,out = net.target_encoder(X_var)
                y_pred = F.softmax(out).data.cpu().numpy()
        ...
        best_mask_rate = 0.0
        best_teacher_model_state = {k: v.cpu().numpy() for k, v in net.target_encoder.state_dict().items()}
        ...
        net.target_encoder.load_state_dict({k: torch.from_numpy(v) for k, v in best_teacher_model_state.items()})
